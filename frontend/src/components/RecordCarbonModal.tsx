...
interface RecordCarbonModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: { dmrvType: string; filesCount: number; totalKwh: number }) => void;
}

export default function RecordCarbonModal({ isOpen, onClose, onSubmit }: RecordCarbonModalProps) {
  const [dmrvType, setDmrvType] = useState('ocr');
  ...
  return (
    <div style={{
      ...
      <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', width: '90%', maxWidth: '450px', textAlign: 'left' }}>
        ...
        {dmrvType === 'ocr' && (
          <div style={{ border: '2px dashed #cbd5e1', padding: '15px', borderRadius: '8px', textAlign: 'center', marginBottom: '15px', backgroundColor: '#f8fafc' }}>
            ...
            <div style={{ backgroundColor: '#eff6ff', padding: '12px', borderRadius: '8px', border: '1px solid #bfdbfe', marginBottom: '10px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
                <button 
                  type="button"
                  onClick={() => setFilesCount(prev => Math.max(1, prev - 1))}
                  style={{ width: '36px', height: '36px', fontSize: '18px', fontWeight: 'bold', backgroundColor: '#e2e8f0', border: 'none', borderRadius: '6px', cursor: 'pointer', color: '#1e293b' }}
                >
                  -
                </button>
                ...
                <button 
                  type="button"
                  onClick={() => setFilesCount(prev => Math.min(10, prev + 1))}
                  style={{ width: '36px', height: '36px', fontSize: '18px', fontWeight: 'bold', backgroundColor: '#2563eb', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}
                >
                  +
                </button>
              </div>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '20px' }}>
          <button 
            onClick={onClose}
            style={{ padding: '10px 16px', backgroundColor: '#e2e8f0', color: '#334155', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}
          >
            Hủy
          </button>
          <button 
...
import { useCarbonContract } from '@/hooks/useCarbonContract'; // Đường dẫn tùy thuộc vào cấu trúc thư mục của bạn

export default function RecordCarbonModal({ isOpen, onClose, onSubmit }: RecordCarbonModalProps) {
  // 1. Gọi hook vừa tạo
  const { claimRewardOnChain } = useCarbonContract();
