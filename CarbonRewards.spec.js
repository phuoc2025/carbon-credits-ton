...
const carbon_rewards_CarbonRewards_1 = require("../build/carbon_rewards_CarbonRewards");
require("@ton/testutils");
describe('CarbonRewards Contract Tests', () => {
    ...
    beforeEach(async () => {
        ...
        carbonRewards = blockchain.openContract(await carbon_rewards_CarbonRewards_1.CarbonRewards.fromInit());
        const deployResult = await carbonRewards.send(deployer.getSender(), {
    ...
    it('nên cập nhật chính xác các chỉ số khi nhận được thông điệp RecordCarbon', async () => {
        ...
        const result = await carbonRewards.send(user.getSender(), {
            value: (0, core_1.toNano)('0.05'),
        }, "RecordCarbon");
